import { NextRequest, NextResponse } from 'next/server';
import OpenAI from 'openai';
import fs from 'fs';
import path from 'path';
import { fetchImages } from '@/app/buildpage/imageProcessor';

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY as string });

interface CacheEntry {
  lastRequest: string;
  lastResult: string;
  timestamp: number;
}

// Simple cache to debounce repeated requests
const generationCache: CacheEntry = {
  lastRequest: '',
  lastResult: '',
  timestamp: 0,
};

export async function generateCustomPage(formData: {
  businessType: string;
  address: string;
  phone: string;
  email: string;
  imageUrls?: string[];
  [key: string]: unknown;
}): Promise<string> {
  const { businessType, address, phone, email, imageUrls = [] } = formData;
  const templateType = businessType.toLowerCase();
  const answers: (string | undefined)[] = [];

  // Create a cache key from the form data
  const requestKey = JSON.stringify(formData);

  // Return cached result if it's recent (within 5 seconds)
  const now = Date.now();
  if (
    requestKey === generationCache.lastRequest &&
    now - generationCache.timestamp < 5000
  ) {
    console.log('🔄 Using cached result...');
    return generationCache.lastResult;
  }

  // Gather all the answers from the form data
  for (let i = 1; i <= 10; i++) {
    const key = `question${i}`;
    answers.push(formData[key] ? String(formData[key]) : '');
  }

  // Create template-specific prompt
  let specificPrompt = '';
  if (templateType === 'restaurant') {
    specificPrompt = `
      - Restaurant name: ${answers[0] ?? address}
      - Cuisine type: ${answers[1] ?? 'Not specified'}
      - Online menu: ${
        answers[2] === 'yes'
          ? 'Include an attractive, well-structured online menu with sections for appetizers, main courses, desserts, and beverages'
          : 'No online menu needed'
      }
      - Reservation system: ${
        answers[3] === 'yes'
          ? 'Include an elegant reservation form with date/time picker and party size selection'
          : 'No reservation system needed'
      }
      - Business hours: ${answers[4] ?? 'Not specified'}
      - Chef/team profiles: ${
        answers[5] === 'yes'
          ? 'Include professional profiles for key staff with high-quality image placeholders'
          : 'No profiles needed'
      }
      - Food gallery: ${
        answers[6] === 'yes'
          ? 'Create a visually appealing masonry-style gallery with lightbox functionality'
          : 'No gallery needed'
      }
      - Testimonials: ${
        answers[7] === 'yes'
          ? 'Include a testimonials carousel with customer quotes and ratings'
          : 'No testimonials section needed'
      }
      - Primary brand color: ${answers[8] ?? '#8D5524'}
      - Delivery/takeout info: ${
        answers[9] === 'yes'
          ? 'Include an online ordering section with delivery radius information'
          : 'No delivery information needed'
      }
    `;
  } else if (templateType === 'logistics') {
    specificPrompt = `
      - Company name: ${answers[0] ?? address}
      - Logistics services: ${answers[1] ?? 'Not specified'}
      - Shipment tracking: ${
        answers[2] === 'yes'
          ? 'Include a professional tracking interface with order ID input field'
          : 'No tracking feature needed'
      }
      - Fleet/equipment showcase: ${
        answers[3] === 'yes'
          ? 'Create a visually appealing carousel showcasing transportation assets'
          : 'No fleet showcase needed'
      }
      - Service areas: ${answers[4] ?? 'Not specified'}
      - Testimonials/case studies: ${
        answers[5] === 'yes'
          ? 'Include a metrics-focused case studies section with client logos and quantifiable results'
          : 'No testimonials section needed'
      }
      - Service request form: ${
        answers[6] === 'yes'
          ? 'Include a multi-step service request form with shipment details fields'
          : 'No service request form needed'
      }
      - Certifications/standards: ${answers[7] ?? 'Not specified'}
      - Primary brand color: ${answers[8] ?? '#1C3D5A'}
      - Service area map: ${
        answers[9] === 'yes'
          ? 'Include an interactive map visualization with service coverage highlighting'
          : 'No map needed'
      }
    `;
  } else if (templateType === 'professional') {
    specificPrompt = `
      - Firm name: ${answers[0] ?? address}
      - Professional services: ${answers[1] ?? 'Not specified'}
      - Team profiles: ${
        answers[2] === 'yes'
          ? 'Include elegant team profiles with professional headshots, credentials, and specialties'
          : 'No team profiles needed'
      }
      - Case studies: ${
        answers[3] === 'yes'
          ? 'Include detailed case studies with problem-solution-result structure'
          : 'No case studies section needed'
      }
      - Client portal: ${
        answers[4] === 'yes'
          ? 'Include a sophisticated client portal login section with secure access messaging'
          : 'No client portal link needed'
      }
      - Consultation info: ${
        answers[5] === 'yes'
          ? 'Feature a prominent consultation booking system with availability calendar'
          : 'Include standard contact information'
      }
      - Credentials/affiliations: ${answers[6] ?? 'Not specified'}
      - FAQ section: ${
        answers[7] === 'yes'
          ? 'Include an accordion-style FAQ section with comprehensive information'
          : 'No FAQ section needed'
      }
      - Primary brand color: ${answers[8] ?? '#2E5984'}
      - Blog/resources: ${
        answers[9] === 'yes'
          ? 'Include a content-rich resources section with categorized articles'
          : 'No blog/resources section needed'
      }
    `;
  }

  const imageInstructions =
    imageUrls.length > 0
      ? `CRITICAL IMAGE INSTRUCTIONS: 
    I have pre-generated these exact image URLs that MUST be used in the website:
    ${imageUrls.map((url, i) => `${i + 1}. ${url}`).join('\n')}
    `
      : 'No custom images provided.';

  const prompt = `
  You are an expert frontend developer specializing in creating visually stunning, conversion-optimized websites. Create a production-ready HTML webpage using the Bootstrap 5 framework for a ${templateType} business.

  BUSINESS DETAILS:
  - Name: ${answers[0] ?? 'Company Name'}
  - Address: ${address}
  - Phone: ${phone}
  - Email: ${email}

  SPECIFIC CONTENT:
  ${specificPrompt}

  IMAGE INTEGRATION:
  ${imageInstructions}

  FOOTER REQUIREMENT:
  Include a footer with multiple columns:
  - Contact information (address, phone, email).
  - Quick links to sections like "Team", "Case Studies", "Resources", and "Contact".
  - Social media links with Font Awesome icons.

  TECHNICAL REQUIREMENTS:
  1. Use Bootstrap 5 CSS framework via CDN for styling.
  2. Embed all custom CSS directly into the <style> tag within the <head> of the HTML file. Do not reference external CSS files like /css/style.css.
  3. Implement a fully semantic HTML5 structure with proper hierarchy.
  4. Include comprehensive meta tags for SEO optimization (title, description, OG tags).
  5. Create a fully responsive design with optimized breakpoints for all devices.
  6. Design a sleek navigation system with smooth scrolling and mobile-friendly dropdown.
  7. Implement ARIA attributes and semantic elements for full accessibility.
  8. Use Font Awesome 6 or Bootstrap Icons via CDN for visual elements.
  9. Create elegant hover animations and transitions using CSS.
  10. Incorporate subtle parallax effects for background elements.
  11. Add schema.org JSON-LD structured data for SEO enhancement.
  12. Implement form validation with user-friendly error handling.
  13. Ensure cross-browser compatibility.
  14. Optimize for page speed (lightweight assets, proper resource loading).
  15. Ensure full keyboard navigation support for accessibility.

  HERO SECTION:
  - Create a visually appealing hero section with a background image, overlay, and a call-to-action button.
  - Include a heading and a short description about the business.

  TEAM PROFILES:
  - Include a "Meet Our Team" section with images, names, titles, and short descriptions for each team member.
  - Use a grid layout with three team members per row.

  CASE STUDIES:
  - Add a "Case Studies" section with images, problem-solution-result descriptions, and a visually appealing layout.
  - Include at least three case studies with meaningful placeholder content.

  TESTIMONIALS:
  - Include a "Testimonials" section with client quotes and names.
  - Use a card layout with three testimonials per row.

  FAQ:
  - Add an accordion-style FAQ section with at least three questions and answers.
  - Ensure the accordion is styled and functional.

  RESOURCES:
  - Include a "Resources & Blog" section with cards for articles, each containing an image, title, short description, and a "Read More" button.
  - Include at least three resource cards.

  ONLY return the complete HTML file with no markdown, explanations, or additional text.
  The code must be production-ready with no placeholders or TODO comments.
  `;

  try {
    console.log('\n🔄 Generating your custom website...');

    const completion = await openai.chat.completions.create({
      model: 'gpt-4o',
      messages: [{ role: 'user', content: prompt }],
      temperature: 0.7,
      max_tokens: 4000,
    });

    let htmlContent: string | null | undefined =
      completion.choices[0]?.message?.content;

    if (htmlContent) {
      // Clean up any remaining markdown code blocks if present
      htmlContent = htmlContent.replace(/```html|```/g, '').trim();

      // Update cache
      generationCache.lastRequest = requestKey;
      generationCache.lastResult = htmlContent;
      generationCache.timestamp = now;

      return htmlContent;
    } else {
      console.error('❌ Error: Generated content is undefined.');
      return 'Error generating content. Please try again.';
    }
  } catch (error) {
    console.error('❌ Error generating page:', error);
    return 'Error generating page. Please check your inputs and try again.';
  }
}

export async function POST(request: NextRequest) {
  try {
    const requestData = await request.json();
    const { imageInstructions, ...formData } = requestData;

    // Process image instructions
    const imageUrls = await fetchImages(imageInstructions || '');

    // Generate HTML with images
    const htmlContent = await generateCustomPage({
      ...formData,
      imageUrls,
    });

    // Save the generated HTML
    const now = new Date();
    const timestamp = `${now.getFullYear()}-${String(
      now.getMonth() + 1
    ).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
    const suffix = `${now.getHours()}${now.getMinutes()}${now.getSeconds()}`;
    const fileName = `${formData.businessType.toLowerCase()}-${timestamp}-${suffix}.html`;

    const outputDir = path.join(process.cwd(), 'gen_comp');
    if (!fs.existsSync(outputDir)) {
      fs.mkdirSync(outputDir, { recursive: true });
    }

    const filePath = path.join(outputDir, fileName);
    fs.writeFileSync(filePath, htmlContent);

    return NextResponse.json({
      htmlContent,
      filePath: `/gen_comp/${fileName}`,
      imageUrls,
    });
  } catch (error) {
    console.error('Error generating page:', error);
    return NextResponse.json(
      { error: 'Error generating page.' },
      { status: 500 }
    );
  }
}
