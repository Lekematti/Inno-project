...
export const useElementHandlers = ({
  ...
  useEffect(() => {
    ...
    // Attach handlers to all matching text elements
    editableElements
      .filter((el) => el.type === 'text' && el.element && el.content)
      .forEach((element) => {
        const candidates = Array.from(doc.querySelectorAll(element.element))
          .filter((el) => el.textContent?.trim() === element.content.trim());
    ...
    // Cleanup: remove event listeners and style on unmount
    return () => {
      // Remove style tag
      if (style.parentNode) {
      ...
      editableElements
        .filter((el) => el.type === 'backgroundImage' && el.selector)
        .forEach((element) => {
          const el = doc.querySelector(element.selector as string) as HTMLElement;
          if (el) {
            el.classList.remove(
              'editable-highlight',
              'bg-image-element',
