...
export const usePageRefreshHandler = ({
  ...
  const getFullStorageKey = useCallback(() => {
    ...
    const contentHash = referenceContent 
      ? btoa(referenceContent.substring(0, 100)).substring(0, 10).replace(/[+/=]/g, '')
      : '';
  ...
  useEffect(() => {
    ...
    if (savedContent && savedContent !== referenceContent && savedContent !== currentContent) {
      // Set flag to prevent this from running again
      hasRestoredRef.current = true;
      
      // We have saved content that differs from both reference and current
      if (confirm('We found unsaved work from your previous session. Would you like to restore it?')) {
  ...
  }, [getFullStorageKey, referenceContent, onRestore, promptRestore, currentContent]);
